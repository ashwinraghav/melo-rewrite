...
def test_record_progress(client):
    _setup_alice(client)
    r = client.post("/v1/me/history/the-whispering-pines", json={"progressSeconds": 60, "completed": False}, headers=auth(USER_ALICE))
...
def test_record_completion(client):
    _setup_alice(client)
    r = client.post("/v1/me/history/the-whispering-pines", json={"progressSeconds": 180, "completed": True}, headers=auth(USER_ALICE))
...
def test_upsert_overwrites_previous_entry(client):
    ...
    _setup_alice(client)
    client.post("/v1/me/history/the-whispering-pines", json={"progressSeconds": 30, "completed": False}, headers=auth(USER_ALICE))
    client.post("/v1/me/history/the-whispering-pines", json={"progressSeconds": 180, "completed": True}, headers=auth(USER_ALICE))
...
def test_history_is_user_isolated(client):
    ...
    client.post("/v1/me/history/the-whispering-pines", json={"progressSeconds": 60, "completed": False}, headers=auth(USER_ALICE))
