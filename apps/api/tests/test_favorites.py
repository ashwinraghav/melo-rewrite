...
def test_add_favorite(client):
    _setup_alice(client)
    r = client.post("/v1/me/favorites/story-animals", headers=auth(USER_ALICE))
    assert r.status_code == 201
    assert r.json()["data"]["storyId"] == "story-animals"


def test_add_favorite_is_idempotent(client):
    _setup_alice(client)
    client.post("/v1/me/favorites/story-animals", headers=auth(USER_ALICE))
    client.post("/v1/me/favorites/story-animals", headers=auth(USER_ALICE))
    r = client.get("/v1/me/favorites", headers=auth(USER_ALICE))
...
def test_remove_favorite(client):
    _setup_alice(client)
    client.post("/v1/me/favorites/story-animals", headers=auth(USER_ALICE))
    r = client.delete("/v1/me/favorites/story-animals", headers=auth(USER_ALICE))
